...
def main():
    ...
    #tb_logger = TensorBoardLogger(save_dir="/home/zhaoxizh/zhaoxizh_project/uflow/result/logs/")
    ...
    if args.n_gpu > 0:
        ...
        trainer = pl.Trainer(accelerator='gpu', devices=args.gpu, default_root_dir=args.save_checkpoint_dir,
                             check_val_every_n_epoch=1, max_epochs=args.num_pre_train_steps)
    else:
        trainer = pl.Trainer(accelerator='gpu',
                            default_root_dir=args.save_checkpoint_dir,
                             check_val_every_n_epoch=1,
                             max_epochs=args.num_pre_train_steps)#args.num_pre_train_steps

    uflow = create_uflow(args)

    trainset = FCTrainDataModule( data_dir=args.pre_train_data_dir,
                                   batch_size=4,
