...
def save_image_as_png(image, filename):
  image_uint8 = TTF.convert_image_dtype(image,torch.uint8)
  image_png = torchvision.io.encode_png(image_uint8)
  torchvision.io.write_file(filename, image_png)
...
def flow_tensor_to_rgb_tensor(motion_image):
  ...
  """
  # sqrt(a^2 + b^2)
  hypot = lambda a, b : (a.type(torch.float32)**2.0 + b.type(torch.float32)**2.0)**0.5
  height, width = motion_image.shape[-2:]
...
def plot_flow(image1, image2, flow, filename, plot_dir):
  ...
  height, width = [float(s) for s in image1.shape[-2:]]
...
def plot_masks(image, masks, filename, plot_dir):
  ...
  height, width = [float(s) for s in image.shape[-2:]]
  plt.figure('plot_flow', [10. * width / (2 * height), 10.])
  plt.clf()
